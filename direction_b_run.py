...
用法：
    ...
    python direction_b_run.py --node-token OfcbwcdeyicTKlkgS6fczBYhn4e \\
        --pkg-id YOUR_PKG_ID_1 --dry-convert

    # 读取+转换+写 CMS（先自动核对 corp_id）
    python direction_b_run.py --node-token OfcbwcdeyicTKlkgS6fczBYhn4e \\
        --pkg-id YOUR_PKG_ID_1 --corp-id YOUR_CORP_ID_5 --write

    # 先写 1 篇探路
    python direction_b_run.py --node-token ... --pkg-id YOUR_PKG_ID_1 --corp-id YOUR_CORP_ID_5 \\
        --write --preflight
...
def main():
    ...
    ap.add_argument("--space-id", default="YOUR_SPACE_ID")
