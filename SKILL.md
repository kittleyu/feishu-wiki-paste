...
@机器人 示例期货公司 10篇
@机器人 示例期货公司 20篇
@机器人 示例客户最新十五篇          # 中文数字也认
@机器人 示例期货公司 5篇 自建个7.22目录   # 显式子目录，覆盖「当日日期」
```
> `client_map.json` 当前已配：示例期货公司 / 示例期货公司 / 示例客户 / 示例肛肠医院 / 示例客户 / 示例客户。加新客户只需告诉我
> 「公司名 → 目录链接 + 表格链接」，我补一行即可。**审核表是可选的**——配了才会自动填。

**也可在对话里/本地直接驱动**（不依赖 @机器人）：
```bash
python run_client.py 示例期货公司 10            # 默认建当日日期子目录
python run_client.py 示例期货公司 20 --subdir 7.22   # 指定子目录
python run_client.py 示例客户 10 --dry-run       # 只预览转换，不写不填
```
...
```js
// 搜索客户
fetch('/yunying/v1/corp?keyword=示例期货公司&page=1&page_size=2')
// 返回 data.corp_list[0].corp.id → corp_id（示例期货公司=YOUR_CORP_ID_2）
```
...
[1/20] 文章A
  → 15 blocks
  ✅ https://YOUR_TENANT.feishu.cn/wiki/xxx
...
[3/20] 文章C
  → 10 blocks
  ✅ https://YOUR_TENANT.feishu.cn/wiki/yyy
...
| 🔴 **用户给的"公司编号"不是 corp_id** | 把内部编号当 corp_id | YOUR_WRONG_ID_2≠真实 YOUR_CORP_ID_5，界面筛不出 | 永远用脚本反查，不信任手写数字 |
...
- **`direction_a_cms_to_feishu.py`** — 🆕 **方向 A 全自动（CMS→飞书）**：登录 huiyouhua → 按公司名解析真实 corp_id（同名多公司时自动选「有文章且最多」的那个）→ `changecorp` → 拉 `creation/articles` 最新 N 篇（含完整 content，按 `created_at` 降序）→ 复用 `batch_paste` 写入目标飞书 Wiki 目录。`python direction_a_cms_to_feishu.py --company '示例期货公司' --node <目录node> --limit 10`（支持 `--dry-run` 预览转换、`--space-id` 指定空间）。`find_corp_by_name()` / `run_pipeline_a()` 均可被 bot 直接调用。`direction_a` 结果 json 含 `urls`（飞书文档链接列表）与 `articles`（标题+正文）。
...
- 方向 A 的公司名后的 `(id：YOUR_WRONG_ID_1)` 等标注会被自动忽略（按名反查 + 探文章数选真实 corp）。
- **指令解析对口语较宽容**（不是死板模板）：①数量「条/篇/篇文章」都识别，中文数字（十/二十）也认，且**不写数量时默认 10 篇**（方向 A）；②公司名后的 `(id:YOUR_WRONG_ID_1)` 标注自动去除；③消息里贴的飞书链接自动抓取（位置不严格，按出现顺序取源/目标）；④解析不出会回群 `⚠️ 没看懂指令`+ 格式示例，**不会静默失败**。硬要求只有：群里必须 **@机器人**。**方向 A 触发有两种**：(a) 含「粘贴」关键字 + 飞书目录链接（任意公司）；(b) **client_map 里已配的公司名 + 篇数**（如 `示例期货公司 10篇`，无需给链接，自动建当日日期子目录并填表，v2.7.0+）。方向 B 要「粘贴」、方向 F 要「填」+「链接/表」。
...
```
示例：
> 把示例期货公司最新 20 篇文章粘贴到 https://xxx.feishu.cn/wiki/ABCD 的 7.10 目录下，
...
|------|---------|--------|-------------------|------|
| 示例口腔诊所 | **YOUR_CORP_ID_5** | 示例口腔医院 | YOUR_PKG_ID_1 | ✅ 已实测验证(2026-07) |
| 示例血管医院 | **YOUR_CORP_ID_3** | 示例静脉曲张医院 | YOUR_PKG_ID_4 | ✅ 已实测验证(2026-07) |
| 示例客户公司 | **YOUR_CORP_ID_1** | 示例词包 | YOUR_PKG_ID_2 | ✅ 已实测验证(2026-07，30篇落库) |
| 示例客户 | YOUR_CORP_ID_1 | 示例词包 | YOUR_PKG_ID_6 | ⚠️ 旧编号，写前务必 verify-corp |
| 示例客户 | YOUR_CORP_ID_6 | 智能股票软件推荐 | YOUR_PKG_ID_7 | ⚠️ 旧编号，写前务必 verify-corp |
| 示例期货公司 | **YOUR_CORP_ID_2** | 示例期货公司词包 | YOUR_PKG_ID_3 | ✅ 已实测验证(2026-07) |
| 示例血管医院 | **YOUR_CORP_ID_4** | 示例静脉曲张医院 | YOUR_PKG_ID_5 | ✅ 已实测验证(2026-07) |
| 示例期货公司 | **YOUR_CORP_ID_0** | （方向 A：CMS→飞书，按文章数自动选） | — | ✅ 实测 1728 篇，方向 A 批量写入飞书(2026-07) |

### 飞书节点速查（方向 A / 填表用）
| 用途 | node_token | 类型 | 说明 |
|------|-----------|------|------|
| 示例客户 文章目录 | **YOUR_DIR_NODE_YN**（标题「7.20」） | docx（可贴文章页） | 方向 A 把文章页贴这里 |
| 示例客户 审核表 | **YOUR_SHEET_NODE_YN**（「示例客户内容审核表」） | **sheet（表格）** | ⚠️ 只能填链接，**不能贴文章页**；父节点 YOUR_PARENT_NODE_A |
| 示例期货公司 文章目录 | **YOUR_DIR_NODE_C** | docx | 方向 A 文章页 |
| 示例期货公司 审核表 | **YOUR_SHEET_NODE_A**（「示例期货公司内容审核表」） | **sheet** | 链接填 B 列（实测 B108:B137） |
| 示例期货公司 文章目录 | **YOUR_DIR_NODE_B** | docx | 方向 A 文章页 |

> ⚠️ **corp_id 是最大坑**：用户口头给的"公司编号"几乎都不是真实 `corp_id`
> （实测用户给 YOUR_WRONG_ID_2，真实是 YOUR_CORP_ID_5；用户给示例期货公司"YOUR_WRONG_ID_1"，真实是 YOUR_CORP_ID_0）。真实 corp_id 必须用下面「步骤 B7」的
> `--verify-corp` / `--auto-corp` 反查，**不要相信任何手写数字**。
> 更省事：直接用 `cms_discover_ids.py --company '公司全名' --pkg '词包名'`
> 通过 API 自动查出 corp_id/pkg_id（见「可复用脚本」）。
>
> 🔴 **同名多公司（双胞胎坑）**：实测「示例期货公司」在 CMS 里竟有 **7 个**
> 同名 corp（id 2414 / 2260 / 1613 / 1612 / 1611 / 1599 / YOUR_CORP_ID_0），其中 6 个是空的，
> **只有 YOUR_CORP_ID_0 有 1728 篇**。方向 A 的 `find_corp_by_name()` 按公司名匹配到所有候选后，
...
resp = await fetch("/yunying/v1/articles", {
    ...
    body: JSON.stringify({
        title: "文章标题",
        content: "<h2>小标题</h2>\n<p>正文内容...</p>",
        corp_id: YOUR_CORP_ID_1,           # 公司 ID
        keyword_package_id: YOUR_PKG_ID_6  # 词包 ID
    })
})
...
python direction_b_cms_write.py --articles converted.json --pkg-id YOUR_PKG_ID_1 --verify-corp

# 3) 写 1 篇探路，确认它出现在词包列表
python direction_b_cms_write.py --articles converted.json \
    --corp-id YOUR_CORP_ID_5 --pkg-id YOUR_PKG_ID_1 --preflight

# 4) 正式铺开，并逐篇校验落库
python direction_b_cms_write.py --articles converted.json \
    --corp-id YOUR_CORP_ID_5 --pkg-id YOUR_PKG_ID_1
...
- **示例口腔诊所 = YOUR_CORP_ID_5**（示例口腔医院词包 YOUR_PKG_ID_1 已实测 20 篇成功落库）
- **示例血管医院 = YOUR_CORP_ID_3**（示例静脉曲张医院词包 YOUR_PKG_ID_4 已实测 20 篇成功落库）
- **示例期货公司 = YOUR_CORP_ID_2**（示例期货公司词包词包 YOUR_PKG_ID_3 已实测 19 篇成功落库）
- **示例血管医院 = YOUR_CORP_ID_4**（示例静脉曲张医院词包 YOUR_PKG_ID_5 已实测 59 篇成功落库，含 6/29·6/30·7.1·7/2 四子目录）
- **示例客户公司 = YOUR_CORP_ID_1**（示例词包词包 YOUR_PKG_ID_2 已实测 30 篇成功落库）
...
| **同名多公司陷阱** | 示例期货公司在 CMS 有 7 个同名 corp，只有 YOUR_CORP_ID_0 有文章。`find_corp_by_name` 自动探文章数选最多的 |
| **用户给的"编号"不是 corp_id** | 示例期货公司用户给 YOUR_WRONG_ID_1，真实 YOUR_CORP_ID_0；永远按名反查 + 探文章数 |
...
corp_id, corp_name = find_corp_by_name(page, "示例期货公司")
# → 打印 7 个候选的文章数，自动选 YOUR_CORP_ID_0（1728 篇）
```
...
res = run_pipeline_a(
    company="示例期货公司",
    node_token="YOUR_DIR_NODE_B",  # 目标飞书 Wiki 目录
    limit=10,                                  # 最新 10 篇
    log_fn=print,
)
# → res = {"total":10,"ok":10,"bad":0,"urls":[...],"corp_id":YOUR_CORP_ID_0,"space_id":...}
```
...
- **示例期货公司 = YOUR_CORP_ID_0**（1728 篇，方向 A 批量写入飞书 2026-07 实测 10/10 成功）
...
| SPACE_ID | `YOUR_SPACE_ID` | 知识库空间 ID |
...
| v2.7.5 | 2026-07-22 | 🐞 **修 run_client.py 路径飞书凭证未注入**：`run_pipeline_a` 内部漏调 `load_env()`（仅 CLI `main()` 调了），经 `run_client.py` 端到端跑方向 A 时 `KeyError: FEISHU_APP_ID`；已在 `run_pipeline_a` 开头补 `load_env()`。🆕 新增客户「示例肛肠医院」到 client_map.json（dir=YOUR_DIR_NODE_D / sheet=YOUR_SHEET_NODE_B），实战 30/30 写入飞书子目录 `7.22` 并回填审核表 |
...
| v2.7.1 | 2026-07-21 | 🐞 **修简写格式被预过滤误拒**：`示例期货公司 20篇` 这类「无动作关键字」指令因预过滤要求含「粘贴/填/生文」被回「没识别动作」；改为 client_map 命中公司名即放行方向 A |
| v2.7.0 | 2026-07-21 | 🚀 **client_map 简写流程（日常 90% 场景）**：新增 `client_map.json`，用户只需 `@机器人 <公司名> <篇数>`（如 `示例期货公司 10篇`）即自动匹配顶层目录 + 建当日日期 M.D 子目录 + 粘贴 + 回填审核表，无需每次给链接；中文数字、公司名清洗一并支持。配套 `run_client.py` 可在对话/本地直接驱动 |
| v2.6.13 | 2026-07-21 | 🔧 **修方向 A 解析**：① 数量支持中文数字（十/二十），`示例期货公司的最新十篇` 正确拆为 company=示例期货公司、limit=10（旧正则只认阿拉伯数字把整串当公司名 → 查不到 corp）；② 支持指令里「自建/新建/下面新建 X 目录」自动建子目录并贴入 |
...
| v2.6.0 | 2026-07-17 | 🆕 **方向 A 全自动跑通（CMS→飞书）**：新增 `direction_a_cms_to_feishu.py`（`find_corp_by_name` 同名多公司自动选有文章的 + `run_pipeline_a` 拉最新 N 篇写入飞书）；固化示例期货公司=YOUR_CORP_ID_0（7 个同名 corp 中唯一有 1728 篇）；`feishu_bot.py` 升级为**双向**，新增方向 A 指令解析（`把 <公司> 粘贴到 <飞书目录> 最新N条`）；实战写入示例期货公司最新 10 篇到飞书目录 YOUR_DIR_NODE_B（10/10 成功） |
| v2.6.1 | 2026-07-20 | 🆕 **链接回填飞书电子表格**：新增 `fill_sheet.py`（走 `sheets/v2`：metainfo 取 sheetId → 读列找末行 → PUT /values 追加链接到指定列），实战把示例期货公司最新 30 篇飞书链接追加到「【GEO服务版-金融】示例期货公司内容审核表 / 7月内容」B108:B137（30/30）；明确 **电子表格(sheet) ≠ 多维表格(bitable)** 两套 API，应用需 `sheets:spreadsheet` 权限+分享表格给机器人 |
| v2.6.2 | 2026-07-20 | 🐛 **修 bot 方向 A 两个根因**：① `parse_command` 数量正则只认「最新N条」，漏掉「最新N篇/最新N篇文章」→ 退回默认 10（这就是「每次只粘 10 条」的真因）；现已用 `最新\s*(\d+)` 提取，且 company 自动去尾部「的最新N篇…」；② **进程级单实例锁** `.bot.lock`（含 pid，存活则拒绝重复启动），杜绝多实例各持一把锁抢 Chrome 9222 打架；③ `process_a` 写前校验目标节点 `obj_type`，若是 sheet/bitable/mindnote 直接回群提示「不能贴文章页，请给文件夹链接」（避免误贴表格节点整批失败）。新增 `paste_yn_missing.py`（按标题去重补贴缺失篇章，实战把示例客户最新 30 篇补满到 YOUR_DIR_NODE_YN，30/30）；新增「飞书节点速查」表 |
| v2.6.3 | 2026-07-20 | 🆕 **机器人新增方向 F（目录文章链接→填表）**：`@机器人 把 <源目录链接> 的文章链接填到 <表格链接> 第二列 [工作表 X]` 即可把源目录全部 docx 子页链接追加写进目标电子表格指定列（默认 B，从已填下一行起）。`fill_sheet.py` 抽出可复用 `run_fill()`；新增 `collect_d5nl_links.py` 收集目录文章链接。实战：示例客户 D5nL 目录 30 篇链接填进「示例客户内容审核表 / 6月内容」B362:B391（30/30，与已有 361 行 0 重复）。同时修 `process_a` 中 `prepare_multi.get_token()` 的模块名引用 bug（改为直接 `get_token()`） |
...
| v2.6.7 | 2026-07-20 | 🔧 **方向 G 改为直接 API 生文（根治公司作用域事故）**：`shengwen_pipeline.py` 由「模拟点击 UI」重构为**直接调 huiyouhua 后端接口**——`corp/active`+`auth/changecorp` 按名定位 corp（同名多 corp 逐个探测，示例期货公司 7 个同名 corp 仅 YOUR_CORP_ID_0 有内容自然命中）→ `knowledge-base` 取 `product_kb_id` → `keyword/package` 取 `keyword_package_id`（词包名称字段是 `core_keyword`，非 `name`）→ `POST /creation/tasks`(`product_kb_id`+`keyword_package_id`+`article_count`，**无 corp_id**，公司由 product_kb_id 决定）→ 轮询 `creation/articles` 筛新文章 → 飞书顶层目录自动建「当天日期 M.D」子目录并 `batch_paste` 粘贴 → 可选 `run_fill` 填表。已验证：示例期货公司=2198/1773/YOUR_CORP_ID_0、示例民宿=4187/5024/2692 解析正确；`connect_cdp` 加 asyncio 重试防护（同进程连续两条 G 指令不崩）；`feishu_bot.py` 的 `process_g` 回发逻辑同步更新 |
...
| v2.6.9 | 2026-07-21 | 🔧 **方向 A 粘贴改为对话内全自动 + 修子目录建块 bug**：① 实测沙箱可**自起调试 Chrome + 自动登录 huiyouhua**（`HYH_PWD` 在 `.env`，`ensure_chrome()` 用临时 profile 拉起并登录），故方向 A（CMS 文章→飞书）可在对话里直接跑完，**不必用户双击**；填表只用飞书 token，更不依赖 Chrome。② 修 `ensure_date_subdir` 建日期子目录的块格式 bug：`block_type:2` 必须用 `"text"` 键（含 `text_element_style`），误用 `"paragraph"` 会被飞书拒 `1770001 invalid param`（示例期货公司这次复现失败，示例期货公司那次侥幸成功）。③ `fill_sheet.run_fill` 新增 `date_label`：第二列填链接、第一列整块合并写日期（如「7月21日」），已用于示例客户/示例期货公司审核表。④ 新增 `run_yongan_paste.py`/`run_yunan.py` 一键驱动模板（方向A 粘贴最新 N 篇 + 建日期子目录 + 填表）。实测：示例期货公司 corp=YOUR_CORP_ID_2，最新 20 篇粘贴进 `YOUR_DIR_NODE_E...` 下 `7.21` 子目录，填 `YOUR_SHEET_NODE_A...`（B138:B157 链接 / A138:A157 合并「7月21日」）全 20/20 成功 |
| v2.4.2 | 2026-07-17 | 🆕 `auto_paste.py` 参数化（去掉示例期货公司硬编码，改 `--company/--pkg/--in` 参数，支持任意公司/词包）；🆕 新增 `prepare_multi.py`：父目录取「指定日期子目录」文章并合并去重（自动归一化分隔符、逐层收集 doc）；固化 示例血管医院=YOUR_CORP_ID_4 / 示例静脉曲张医院=YOUR_PKG_ID_5（59 篇实测落库，含 6/29·6/30·7.1·7/2 四子目录）；`.gitignore` 增加 `*_articles.json`/`*_cms_write_results.json` 通用忽略 |
| v2.4.0 | 2026-07-15 | 🆕 新增 `cms_discover_ids.py`：通过 API 自动反查公司 corp_id + 词包 pkg_id（corp/active 列公司 + auth/changecorp 切换 + keyword/package 取词包），彻底免盲猜；实测示例血管医院=YOUR_CORP_ID_3 / 示例静脉曲张医院=YOUR_PKG_ID_4 |
| v2.3.0 | 2026-07-15 | 🆕 方向 B 实战踩坑固化：新增 `launch_chrome_debug.py`/`direction_b_cms_write.py`/`direction_b_run.py`；补充 corp_id 陷阱(corp_id 错→界面看不见)、Chrome 调试端口必须非默认配置两大坑；新增 `--verify-corp`/`--auto-corp`/`--preflight` 防错机制；实测 corp_id 示例口腔诊所=YOUR_CORP_ID_5 |
