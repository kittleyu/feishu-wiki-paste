...
| 方向 | 含义 | 触发示例 |
|---|---|---|
| **A** | CMS 已审核文章 → 飞书 Wiki 目录（自动建当日日期子目录）+ 回填审核表 | `示例期货公司 30篇` |
| **G** | 智能生文：调 huiyouhua 后端接口生文 → 粘贴飞书 + 填表 | （机器人指令） |
| **B** | 飞书 Wiki 文章 → 回贴 CMS 词包 | `https://...wiki/XXX 粘贴回示例客户 示例词包词包下` |
| **F** | 把飞书目录下的文章链接批量填入审核表 | `把 https://.../AvgPws... 下的文章填到示例客户审核表` |
